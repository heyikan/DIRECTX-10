...
ID3D10EffectMatrixVariable* g_pWorldInverseTransposeVariable = NULL;
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	wcex.hIcon = LoadIcon(hInstance, (LPCTSTR)IDI_DIRECTX_ICON);
	wcex.hCursor = LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = NULL;
	wcex.lpszClassName = L"TriangleClass";
	wcex.hIconSm = 0;
	...
	g_hWnd = CreateWindow(L"TriangleClass", L"DIRECTX 10 ve SHADER PROGRAMLAMA : ÜÇGEN ÇÝZÝMÝ", WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
	...
	g_pWorldInverseTransposeVariable = g_pEffect->GetVariableByName("g_WorldInverseTranspose")->AsMatrix();
	g_pWorldVariable = g_pEffect->GetVariableByName("World")->AsMatrix();

	g_pViewVariable = g_pEffect->GetVariableByName("View")->AsMatrix();
	g_pProjectionVariable = g_pEffect->GetVariableByName("Projection")->AsMatrix();

	g_pLightPosVariable = g_pEffect->GetVariableByName("vLightPos")->AsVector();
	...
	{
		MessageBox(NULL,
			L"Vertex Buffers wasn't created", L"Error", MB_OK);
		return hr;
	}
	...
	{
		MessageBox(NULL,
			L"Index Buffers wasn't created", L"Error", MB_OK);
		return hr;
	}
	...
	D3DXMatrixPerspectiveFovLH(&g_Projection, (float)D3DX_PI * 0.25f, 1.33f, 0.1f, 100.0f);
...
void Render()
{
	...
	// UCGENI Y EKSENI ETRAFINDA DONDUR
	...
	{
		D3DXVECTOR4(-3.0f, 0.0f, 0.0f, 1.0f),
		D3DXVECTOR4(0.0f, 0.0f, -3.0f, 1.0f),
	};


	D3DXVECTOR4 vLightColors[2] =
	{
		D3DXVECTOR4(1.0f, 0.0f, 0.0f, 1.0f), // Kýrmýzý ýþýk kaynaðý
		D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f), // Mavi ýþýk kaynaðý
	};
	...
	D3DXMATRIX mRotate;
	D3DXMatrixRotationZ(&mRotate, t);
	D3DXVec3Transform(&vLightPos[0], (D3DXVECTOR3*)&vLightPos[0], &mRotate);
	D3DXMatrixRotationY(&mRotate, t);
	D3DXVec3Transform(&vLightPos[1], (D3DXVECTOR3*)&vLightPos[1], &mRotate);
	...
	D3DXMatrixInverse(&g_WorldInverseTranspose, 0, &g_World);
	...
	for (int m = 0; m < 2; m++)
	{
		...
		D3DXMatrixTranslation(&mLight, vLightPos[m].x, vLightPos[m].y, vLightPos[m].z);
